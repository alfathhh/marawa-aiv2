...
- ✅ **Action masking (docs/27 reference)**: `scripts/action_masking.py` — `allowed_actions(state, has_selected)` + `apply_action()` + `mask_prompt_line()`; server membatasi action model per conversational state (query fakta terkunci sampai kandidat dipilih; QUEUED melarang balasan layanan; ADMIN_ACTIVE = bot diam; engine-actions tak pernah jadi opsi model). 6 tes. **Pakai di eval**: hasil kedua — **0 pelanggaran masking, 0 call-failed**; sisa kegagalan = nuansa murni (fixture mengharapkan `clarify`, model langsung eksekusi). Model produksi switch ke **`gemini-3.5-flash-lite`** (usable: json_schema+tools+tolak halusinasi); `gemini-3.5-flash` (full, lebih lambat) sedang diuji banding.

...
- ✅ **Eval LLM LIVE (OQ-05 resolved)**: `eval_golden_episodes --mode llm` memainkan 19 episode lawan model (retry/backoff utk rate limit, event-only turns di-skip → tugas session-policy engine, bukan model). **Hasil jujur (action-only, tanpa masking): 4/19.** **Temuan 1**: tanpa state, model tak bisa menebak nuansa multi-turn.
- ✅ **Action masking (docs/27 reference)**: `scripts/action_masking.py` — `allowed_actions(state, has_selected)` + `apply_action()` + `mask_prompt_line()`; server membatasi action model per conversational state (query fakta terkunci sampai kandidat dipilih; QUEUED melarang balasan layanan; ADMIN_ACTIVE = bot diam; engine-actions tak pernah jadi opsi model). 6 tes.
- ✅ **Eval LLM HYBRID (observation + tool-calling + masking)**: offering engine dijalankan server-side → papan kandidat NYATA masuk prompt; tool `offer_candidates` tersedia ke model (Gemini replay butuh `thought_signature`/raw object — ditemukan & fixed: 400 "missing thought_signature"); selection dari user text ref (`D1/S1/...` = pilih, bukan nunggu model); forbidden `bot_reply_during_admin_queue` dicek saat state QUEUED. **Skor HARD 18/18 clean (0 forbidden/masking/infra fail), 1 not-evaluated, status `ran`**; soft mismatch 28 → kategori review MANUSIA (anti-overfit). Sebelum hybrid: 4/19 & 12 infra fail.
- ✅ **#6 Probe + model produksi**: `gemini-3.5-flash-lite` — usable (json_schema, tools, tolak halusinasi), latency ~0,8s. `gemini-3.5-flash` full: 3/19 di eval → lite dipilih.
